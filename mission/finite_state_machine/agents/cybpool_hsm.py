...
class SearchForTarget(State):
	...
	def objectDetectionCallback(self, msg):
		...
		"""
		#Position of object in cameraframe
	...
	def execute(self, userdata):
		...
		if self.object_px >= 0.0 and self.object_py >= 0.0: #If object is in frame => its found
			counter = 0
...
class TaskManager():
	...
	def __init__(self):
		...
		with sm_gate_tasks:
			...
			StateMachine.add('SCANNING_OBJECTS', SearchForTarget('gate'), transitions={'found':'CAMERA_CENTERING','unseen':'SCANNING_OBJECTS','passed':'','missed':''},
		...
		with hsm_square_patrol:
			...
			StateMachine.add('DP_POS_P1',ControlMode(POSE_HEADING_HOLD),transitions={'succeeded':'POS_P1','aborted':'TRAVEL_P1','preempted':'TRAVEL_P1'})
			StateMachine.add('POS_P1',nav_terminal_states['point1'],transitions={'succeeded':'TRANS_P2','aborted':'TRAVEL_P1','preempted':'TRAVEL_P1'})
			StateMachine.add('TRANS_P2',ControlMode(OPEN_LOOP),transitions={'succeeded':'TRANSIT_TO_GATE','aborted':'TRAVEL_P1','preempted':'TRAVEL_P1'})
			...
			StateMachine.add('DP_POS_P2',ControlMode(POSE_HEADING_HOLD),transitions={'succeeded':'POS_P2','aborted':'TRAVEL_P1','preempted':'TRAVEL_P1'})
			...
			StateMachine.add('DP_POS_P3',ControlMode(POSE_HEADING_HOLD),transitions={'succeeded':'POS_P3','aborted':'TRAVEL_P1','preempted':'TRAVEL_P1'})
